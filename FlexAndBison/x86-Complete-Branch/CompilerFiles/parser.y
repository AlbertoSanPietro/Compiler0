...
%type <node> Program Stmt BlockItem BlockItemList Decl Expr
...
Function:
      INT MAIN '(' ParamListOpt ')' '{' BlockItemList '}'   { $$ = make_function("main", $4, $7); }
    | INT IDENT '(' ParamListOpt ')' '{' BlockItemList '}'  { $$ = make_function($2, $4, $7); }
    | VOID IDENT '(' ParamListOpt ')' '{' BlockItemList '}' { $$ = make_function($2, $4, $7); }
;
...
BlockItemList:
      BlockItem                        { $$ = make_stmt_list(NULL, $1); }
    | BlockItemList BlockItem          { $$ = make_stmt_list($1, $2); }
;
...
Decl:
      INT IDENT ';'                    { $$ = make_var_decl($2, NULL); }
